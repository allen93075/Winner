...
def run_randomForests(DataType, X_train, X_test, y_train, y_test, X_pred, y_pred):
    # RF
    rf = RandomForestClassifier(n_estimators=100, random_state=39, max_depth=5, min_samples_split=10,
    ...
    return DataType, TrainAccuracy, TestAccuracy, PredTestAccuracy, pred2, X_test.index, X_test['open'], X_test['high'], \
           X_test['low'], X_test['close'], X_test['volume']
...
def rf_main(data):
    ...
    data['MACD'], data['MACDSIGNAL'], data['MACDHIST'] = talib.MACD(data['close'], fastperiod=3, slowperiod=6,
                                                                    signalperiod=4)
